...
{
	public String validate(int userid) throws SQLException
	{
		ResultSet rs=null;
		...
		{
			rs=usrdao.login(userid);
		
			if(rs.next()!=false)
			{
					String username=rs.getString(1);
					
					System.out.println("came into user return"+username+"---"+rs.getString(1));
			...
			{
				//invalid user//try after registering
				return "Seems you have not registered with us! Please Login after registering yourself!";
			}
		}
		else
		{
			//all slots are filled
			return "OOPS! Sorry..All Parking Slots are filled currently! Try after some time";
		}
	}
	
	public List<ParkingBean> updatefare(int userandom)
	{
	...
	public int[] calculateFareForMonth(int month)
	{
		Admindao admindao=new Admindao();
		int[] result=new int[3];
		
		result=admindao.calculateTotalFare(month);
	...
	public List<ParkingBean> fetchHistory(int userId)
	{
		Admindao admindao=new Admindao();
		List<ParkingBean> holdinglist=admindao.fetchHistoryOfUser(userId);
		if(holdinglist!=null)
			return holdinglist;
		else
			return null;
