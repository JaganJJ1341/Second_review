...
public class Admindao {
	...
	public void calculateTotalFare(int month)
	{
		...
		try{
			...
			while(resultset.next())
				System.out.println("\nTotal Fare collected for this month is: Rs. "+resultset.getInt(1)+"\n Total Number of Parking:"+resultset.getInt(2));
			
			s="SELECT COUNT(USERID) FROM T_XBBNHGZ_USERINFO";
			stmt=c.prepareStatement(s);
			resultset=stmt.executeQuery();
			while(resultset.next())
				System.out.println("\nTotal number of registered users : "+resultset.getInt(1));
			
			/*s="begin hello5566(3,'Ashok'); end;";
			stmt=c.prepareStatement(s);
			stmt.execute();*/
	...
	public int fetchHistoryOfUser(int userId)
	{
		try{
		String s="select RANDOM_ID from T_XBBNHGZ_PARKINFO WHERE USER_ID=?";
		List<ParkingBean> holdingList;
		int count=0;
			...
			while(resultset.next())
			{
				...
				holdingList=pdao.getDetails(resultset.getInt(1));     
